...
function confirm() {
    ...
    router.delete(`/doctors/${props.doctorId}`, {
        onSuccess: () => {
            close();
            emit('success');
        },
...
<template>
    <Modal :model-value="modelValue" title="تأكيد الحذف" size="sm" @update:model-value="emit('update:modelValue', $event)" @close="close">
        <div class="flex items-start gap-3">
            ...
            <div>
                <p class="font-medium text-hospital-text">حذف الطبيب</p>
                <p class="mt-1 text-sm text-hospital-text-3">هل أنت متأكد من حذف هذا الطبيب؟ لا يمكن التراجع عن هذا الإجراء.</p>
            </div>
        </div>
