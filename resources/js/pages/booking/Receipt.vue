...
<template>
    ...
    <div
        class="min-h-screen bg-white p-8 font-sans text-sm text-gray-800 print:p-4"
    >
        <!-- Header -->
        ...
        <div class="mb-6 text-center">
            <span
                class="rounded-full border-2 border-hospital-primary px-6 py-1.5 text-base font-bold tracking-wide text-hospital-primary uppercase"
            >
                إيصال استلام
            </span>
        </div>
        ...
        <div class="mb-6 grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
            ...
            >
                <span class="font-semibold text-hospital-text-2"
                    >رقم الملف:</span
                >
                <span class="font-bold text-hospital-primary">{{
                    booking.file_no
                }}</span>
            </div>
            <div
        ...
        <table class="mb-6 w-full border-collapse text-sm">
            ...
            <tbody>
                ...
                <tr
                    v-if="Number(booking.discount) > 0"
                    class="border-b border-gray-100 text-hospital-success"
                >
                    <td class="p-2">خصم</td>
                ...
                <tr
                    v-if="Number(booking.ins_amount) > 0"
                    class="border-b border-gray-100 text-hospital-accent"
                >
                ...
                <tr
                    class="border-b border-gray-100 bg-hospital-primary-pale font-bold"
                >
        ...
        <p class="mb-6 text-center text-xs text-hospital-text-2">
            طريقة الدفع:
            <strong>{{
                payMethodLabels[booking.pay_method] ?? booking.pay_method
            }}</strong>
        </p>

        <!-- Signature -->
        <div
            class="mt-12 flex justify-between border-t border-dashed border-gray-300 pt-4 text-xs text-hospital-text-3"
        >
        ...
        <div class="mt-6 text-center">
            <FileNoBarcode :value="booking.file_no" />
        </div>
